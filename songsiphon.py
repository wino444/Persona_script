...
# ---- ค่าเริ่มต้น ----
DEFAULT_QUALITY = "192"
DEFAULT_FORMAT = "mp3"   # ตัวเลือก: mp3, m4a, mp4
...
class Config:
    ...
    def get_ydl_opts(self, temp_dir: Path):
        """สร้าง opts สำหรับ yt-dlp ตามรูปแบบที่เลือก"""
        fmt = self.format
        opts = {
            'quiet': True,
            'no_warnings': True,
        ...
        if fmt in ['mp3', 'm4a']:
            # โหมดเสียง: เลือก bestaudio และแปลงด้วย FFmpeg
        ...
        else:  # mp4 (วิดีโอ+เสียง)
            # ดาวน์โหลด bestvideo+bestaudio และ merge เป็น mp4
            opts.update({
    ...
    def _download_single(self, url: str, temp_dir: Path) -> bool:
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                # ถ้าเป็น mp4 จะมีไฟล์ .mp4 อยู่แล้ว (merge แล้ว)
                if ext == 'mp4':
                    # ไฟล์จะลงท้ายด้วย .mp4
